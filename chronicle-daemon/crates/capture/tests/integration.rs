...
async fn capture_engine_delivers_frames() {
    ...
    assert!(
        result.is_none(),
        "Channel should close after engine is dropped"
    );
}
...
async fn capture_engine_finds_displays() {
    ...
    let (mut engine, _receiver) =
        CaptureEngine::start(config).expect("Failed to start — no displays or no permission");
...
async fn encode_captured_frame_as_heif() {
    ...
    let (engine, mut receiver) =
        CaptureEngine::start(config).expect("failed to start capture engine");
    ...
    encode_heif(frame.sample_buffer.inner(), &path, 0.65).expect("HEIF encoding failed");

    assert!(path.exists(), "HEIF file was not created");
    let metadata = std::fs::metadata(&path).unwrap();
    assert!(
        metadata.len() > 1000,
        "HEIF file suspiciously small: {} bytes",
        metadata.len()
    );
