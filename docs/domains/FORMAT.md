...
written against. Where this file and the design disagree, the design wins and
...
`Status` is `Decided <date>`, optionally followed by one or more clauses, each
`, edited <date>`, `, revised <date>` or `, reviewed <date>`. The three say how
...
- `edited` — the file changed on that date and no one recorded what the change
  meant. Use it when the only evidence is that the file differs, such as a
  timestamp, and not that someone judged its content.

No check parses past the label, so this grammar is for readers, not for
scripts.
...
Design §2.1 puts both inside Boundary, and closure check 3 is worded against the
...
requires every bullet to match or be a sentinel, and fails on anything else.
...
`## Why transcription Is Not A Dependent`, because the rule above forbids a bare
prose sentence inside a claim section, which leaves an explanation of an *absence*
nowhere legal to go.
...
`**Code:**` is the design's §2.1 field and stays as the human-readable summary.
`## Owned Files` is the machine-readable list check 8 parses: one exact path per
bullet, copied verbatim from `INVENTORY.txt`. No directories, no globs, no
comma-separated lists. Check 8 does set comparison, so a directory is a failure
...
both naming the same owner, and the owner is one of the two endpoints. The
synthesis's seam table is keyed on the ID, and check 5 joins on it.
...
count reports three for each of those and fails them, and the tempting repair is
to loosen the two-file rule — which is the rule check 5 exists to enforce.
...
The first line of the synthesis is exactly one of these, and it must match what
