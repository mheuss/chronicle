...
- **[2026-09-21] The synthesis's `search` row conflated the register's Roots
  ...
  **Superseded 2026-09-23:** check 5 now compares each row's Outcome with the
  register, and an undecided row must leave it empty. It also compares where the
  row ended up: the file for a confirmed row, and the register's detail up to an
  em dash for the others. The explanation after the dash is still unchecked, so
  this defect still would not be caught.
...
- **[2026-09-21] Two dead branches in check 6 now have falsification cases.**
  ...
  **Superseded 2026-09-22:** fourteen cases were added for check 1, which until
  ...
  control.
  **Superseded 2026-09-23:** the PR #54 review added seventeen more. The harness is
  at 74 cases, 73 bit, one no-op control.
...
- **[2026-09-23] CodeRabbit's review of PR #54 found three checks that accepted
  ...
  Offshoots Filed and Owned Files. The count also catches a repeat that differs
  only in spacing or closing hashes, since Markdown renders those the same. Check 1 now also rejects a candidate name or
