...
| `docs/use-cases/ipc-compat.md` | Five solutions for daemon/UI version skew | accurate as patterns, and now all in this domain or unowned — three name `DaemonConnection.swift` and `ConnectionSession.swift`, which this domain owns; two name `TranscriptionAlertState.swift` and `TranscriptionBannerCopy.swift`, which no confirmed domain owns. None names `crates/ipc/src/retention.rs`, the file the candidate had left |
...
| `.claude/audit/api-contracts.md:3-8` | Scopes the IPC contract to `crates/ipc/src/lib.rs`, `server.rs`, `ipc_handler.rs` and `DaemonConnection.swift` | accurate — four of this domain's five files, and the closest existing statement of this boundary |
