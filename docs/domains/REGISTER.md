...
filed by a sitting and some already existed and were labelled by one, which is
why their numbers are not contiguous.

MVF-1 walks the lowest-numbered row with an empty Outcome that is either not
blocked, or blocked on a candidate that is now decided — any of `confirmed`,
`merged`, `dropped`, `split`. A blocked row becomes walkable the moment its
target has an outcome; it is not excluded for good. The walk repeats until no
row has an empty Outcome and Blocked rows is empty.
...
Every cell below is parsed by closure check 1, so the forms are exact. Per
Design §1.2.
...
`Roots` and `Symbols` are separated by `; ` — semicolon then space. Roots are written relative to
`chronicle-daemon/` or `chronicle-ui/`; a command run from the repo root needs
that prefix, and `grep` fails silently without it. A Symbols cell reads
...
stays empty and the row is recorded here naming what it waits on. The walk
returns to it once the target is decided. A blocked row is incomplete, not
decided — check 1 counts it as incomplete, so a walk still holding one cannot
close. Per Design §1.4.

| # | Candidate | Waiting on | Since | Offshoots |
|---|---|---|---|---|

A blocked sitting writes no Outcome, so check 1's Offshoots rule cannot see it.
...
dropped sitting writes no domain file and its Detail cell holds one sentence, so
...
the reasons, and it requires them non-empty. Until the plan is amended, whoever
writes the synthesis copies the reasons from here by hand.

For the 2026-09-21 walk this never fired: nothing was dropped, so this table stayed
empty and the synthesis's exclusion table is empty too. The gap is recorded under
...
- **[2026-09-21] The walk is closed.** All fourteen rows are decided — eleven
  `confirmed`, three `merged`, none split, none dropped — and no row is blocked.
  ...
  the synthesis's Exclusions table is empty. That is machine-checked, not asserted:
  `checks/check8_coverage.sh` regenerates the inventory from the repo and fails if
  any path is owned twice, owned by nobody, or both owned and excluded. It is what
  settles the hand-applied `except` clauses in rows 4, 9 and 14, and it is why a
  path appearing in several rows' Roots is not a contradiction — Roots scope a
  sitting's searches, they do not assign ownership. `## Owned Files` in each domain
  file does that.
  ...
  **Where the map lives, settled 2026-09-22.** For the length of the walk
  ...
  `DECISIONS.md`, which is chronological and has no index.

  - **Five plan defects**, found during execution and deferred to an amendment that
    was never written. Recorded in `DECISIONS.md` under the 2026-09-16 entries: the
    Task 23 walkability rule, `## Exclusions pending` reassignment, Task 28's
    exclusion-reason gap, two "Task 25 revisits it" mislabels, and Task 26 Step 3's
    missing split path.
  - **BR-6's status line gates one claim and asserts two.** The `Authoritative`
    string says the walk is complete *and* the closure checks pass, while check 5
    selects it on check 1's completeness alone. Both halves are true today. The
    string is fixed by the design, which is inside the protected set, so this is
  ...
  domain files or `DECISIONS.md`, and each one cites a path and a symbol, so it is
  re-derivable from the repo. What did not survive is the per-sitting search
  provenance — which dependency search produced a given bullet, and what the
  negative searches returned. Nothing that remains depends on it.
